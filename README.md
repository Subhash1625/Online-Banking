## End To End Projects Development Steps

- Step 1:
    Creating New Virtual Environment. <br>
        ```Conda Create -n flask_env Python==3.13 -y```
- Step 2:
    Activate The Virtual Environment.
        cmd: ```Conda activate flask_env``` <br>
        git bash: ```Source activate flask_env```
- Step 3:
    Create Following  File Floder Structured:
    -    ```app.py```  For Flask App <br>
    -   ```requirements.txt```For Pre-requirments Moduls <br>
    -   ```templates``` folder - For HTML Files <br>
    -   ```static``` folder - For Styling Files <br>
    -   ```gitignore``` <br>
    -   ```README.md``` <br>
- Step 4:
    Install Pre-requirment Moduls. <br>
    ```pip Install -r requriments.txt```